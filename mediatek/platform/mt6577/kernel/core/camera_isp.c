    ...
    unsigned long   arg)
{
    ...
    if(ret < 0)
    {
        LOG_ERR("CurCmd(%d), process(%s), pid(%d), tgid(%d)\n",IspDebugInfo.CurCmd,current->comm, current->pid, current->tgid);
    }
    //
    ...
    struct file*    file)
{
    ...
    {
        spin_unlock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
        LOG_WRN("One more user:process(%s), pid(%d), tgid(%d)\n",current->comm, current->pid, current->tgid);
        goto EXIT;
    }
    else
    {
    ...
    spin_unlock_irq(&IspSpinlock[ISP_SPINLOCK_IRQ]);
    LOG_MSG("User:process(%s), pid(%d), tgid(%d)\n",current->comm, current->pid, current->tgid);
    //
    ...
    struct file*    file)
{
    ...
    {
        LOG_WRN("No user before:process(%s), pid(%d), tgid(%d)\n",current->comm, current->pid, current->tgid);
        goto EXIT;
    }
    //Force to disable and clear INT when close.
    ...
    void*       data)
{
    ...
    p += sprintf(p, "IspDebugInfo:LogMask(0x%08X),CurCmd(%d),IsWorkQueue(%d),IsTasklet(%d)\r\n",IspDebugInfo.LogMask,IspDebugInfo.CurCmd,IspDebugInfo.IsWorkQueue,IspDebugInfo.IsTasklet);
