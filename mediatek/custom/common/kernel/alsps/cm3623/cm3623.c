...
static void cm3623_prox_calibration(struct i2c_client *client);
/*----------------------------------------------------------------------------*/
...
{
	struct cm3623_priv *obj = i2c_get_clientdata(client);
	u8 prox_min, prox_data[5];
	u8 prox_thd;
	int err;
	int i;

	//enable ps
	cm3623_enable_ps(client, 1);
	mdelay(60);

	prox_min = 255;
	for(i = 0; i < 10; i++)
	{
		if((err = cm3623_read_ps(client, &prox_data[i])))
		{
		...
		if(prox_data[i] < prox_min)
			prox_min = prox_data[i];

		//printk("XXXXXXX prox-val = %d XXXXXXX\n", prox_data[i]);
		mdelay(60);
	}
	prox_thd = prox_min + 4;

	if(prox_min > 110 || 0 == prox_min) //modify by yuanjun for white issue.
		atomic_set(&obj->ps_thd_val, obj->hw->ps_threshold);
	else
		atomic_set(&obj->ps_thd_val, prox_thd);

	//disable ps
	cm3623_enable_ps(client, 0);
	mdelay(60);
}
...
static int cm3623_init_client(struct i2c_client *client)
{
	...
	cm3623_prox_calibration(client);
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				{
					//printk("XXXXXX prox-val = %d XXXXXXX\n", obj->ps);
					sensor_data->values[0] = cm3623_get_ps_value(obj, obj->ps);
					sensor_data->value_divide = 1;
					sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
				}				
...
static int cm3623_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	{
	  atomic_set(&obj->ps_cmd_val,  0xF1);// disable interrupt
