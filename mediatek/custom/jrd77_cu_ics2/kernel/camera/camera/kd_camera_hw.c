...
if (0 == strcmp("ov5647",currSensorName)) {
		...
		pinSet = hi542;
		printk("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX pinSet == hi542 XXXXXXXXXXXXXXXXXX");
}
    ...
    if (On) {
        ...
        if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
			...
			if (0 == pinSetIdx || 2 == pinSetIdx) {
				if(mt_set_gpio_mode(GPIO51, GPIO_MODE_00))
					PK_DBG("[CAMERA SENSOR AF ] set gpio mode failed!! \n");
				if(mt_set_gpio_dir(GPIO51,GPIO_DIR_OUT))
					PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");
				if(mt_set_gpio_out(GPIO51,GPIO_OUT_ONE))
        ...
        if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST]) {
			...
			if (0 == pinSetIdx || 2 == pinSetIdx) {
				...
				if(mt_set_gpio_dir(GPIO51,GPIO_DIR_OUT))
					PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");
				if(mt_set_gpio_out(GPIO51,GPIO_OUT_ZERO))
					PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");
			}
